...
static void drawTitleMenu(void)
{
    ...
    for (int i = 0, x = 22; i < 6; i++) {
        uint8_t *pImg = (uint8_t *)(letterInfos[i].pImg);//TODO:
...
{
    // eeprom_busy_wait();
    // uint16_t ret = myeepr.read(eepAddr);
    uint8_t ret1 = myeep.readByte(eepAddr);
    eepAddr++;
    uint8_t ret2 = myeep.readByte(eepAddr);
    eepAddr++;
    uint16_t ret = (uint16_t)ret1 << 8 | (uint16_t)ret2;
    // uint16_t ret = EEPROM.readByte(eepAddr);
    // eepAddr += 2;
    return ret;
}
...
{
    // eeprom_busy_wait();
    uint16_t ret1 = eepRead16();
    uint16_t ret2 = eepRead16();
    uint32_t ret = (uint32_t)ret1 << 16 | (uint32_t)ret2;
    return ret;
}
...
{
    // eeprom_busy_wait();
    // myeepr.write(eepAddr,val);
    myeep.writeByte(eepAddr,(byte)(val>>8));
    eepAddr++;
    myeep.writeByte(eepAddr,(byte)(val&0x00ff));
    eepAddr++;
    // EEPROM.writeUShort(eepAddr, val);
    // eepAddr += 2;
}
...
{
    // eeprom_busy_wait();
    eepWrite16((uint16_t)((val>>16)&0x0000ffff));
    eepWrite16((uint16_t)(val&0xffff));
}

void eepWriteBlock(const void *p, size_t n)
{
    // eeprom_busy_wait();
    // EEPROM.writeBytes(eepAddr,p,n);
