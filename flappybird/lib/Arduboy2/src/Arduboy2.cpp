...
void Arduboy2Base::systemButtons()
{
  while (pressed(B_BUTTON)) {
    ...
    sysCtrlSound(UP_BUTTON + B_BUTTON, 0, 0xff);
    sysCtrlSound(DOWN_BUTTON + B_BUTTON, 0, 0);
    delayShort(200);
  }

  digitalWriteRGB(0, RGB_OFF); // turn off blue LED
}
...
void Arduboy2Base::sysCtrlSound(uint8_t buttons, uint8_t led, uint8_t eeVal)
{
  if (pressed(buttons)) {
    ...
    EEPROM.write(eepromAudioOnOff, eeVal);
    delayShort(500);
...
bool Arduboy2Base::bootLogoShell(void (&drawLogo)(int16_t))
{
  ...
  for (int16_t y = -15; y <= 24; y++) {
    ...
    if (showLEDs && y == 4) {
      digitalWriteRGB(0, RGB_OFF);    // red LED off
      digitalWriteRGB(0, RGB_ON);   // green LED on
    }
  ...
  if (showLEDs) {
    digitalWriteRGB(0, RGB_OFF);  // green LED off
    digitalWriteRGB(0, RGB_ON);    // blue LED on
  }
  delayShort(400);
  digitalWriteRGB(0, RGB_OFF);
...
{
  EEPROM.write(eepromUnitID, (uint8_t)(id & 0xff));
  EEPROM.write(eepromUnitID + 1, (uint8_t)(id >> 8));
}
...
void Arduboy2Base::writeShowBootLogoFlag(bool val)
{
  ...
  EEPROM.write(eepromSysFlags, flags);
}
