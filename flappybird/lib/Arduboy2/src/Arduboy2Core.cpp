...
uint8_t Arduboy2Core::buttonsState()
{
  ...
  if (digitalRead(PIN_UP_BUTTON) == 0) { buttons |= UP_BUTTON; }
  if (digitalRead(PIN_DOWN_BUTTON) == 0) { buttons |= DOWN_BUTTON; }
  if (digitalRead(PIN_LEFT_BUTTON) == 0) { buttons |= LEFT_BUTTON; }
  if (digitalRead(PIN_RIGHT_BUTTON) == 0) { buttons |= RIGHT_BUTTON; }
  // A
  if (digitalRead(PIN_A_BUTTON) == 0) { buttons |= A_BUTTON; }
  // B
  if (digitalRead(PIN_B_BUTTON) == 0) { buttons |= B_BUTTON; }
#elif defined(AB_DEVKIT)
  // down, left, up
  buttons = ((~PINB) &
...
{
  cli();
  esp_restart();// set bootloader magic key
